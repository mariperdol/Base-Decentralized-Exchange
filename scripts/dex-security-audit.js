// base-dex/scripts/security-audit.js
const { ethers } = require("hardhat");
const fs = require("fs");

async function performDEXSecurityAudit() {
  console.log("Performing security audit for Base Decentralized Exchange...");
  ...
  try {
    ...
    // Оценка уязвимостей
    const vulnerabilityAssessment = await dex.getVulnerabilityAssessment();
    ...
    // Матрица рисков
    const riskMatrix = await dex.getRiskMatrix();
    ...
    if (securityReport.securityControls.encryption === false) {
      securityReport.recommendations.push("Enable data encryption for trading data");
    }
    
    // Сохранение отчета
    const auditFileName = `dex-security-audit-${Date.now()}.json`;
    ...
    console.log("DEX security audit completed successfully!");
